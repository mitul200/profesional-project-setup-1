...
export const creatAcadamicSemesterValidationSchema = z.object({
  ...
  year: z.string(), // Ensures the value is a valid Date object
...
export const updateAcadamicSemesterValidationSchema = z.object({
    body:z.object({
  name: z.enum([...AcadamicSemesterName] as [string, ...string[]]).optional(), // Ensures only these values are allowed
  code: z.enum([...AcadamicSemesterCode] as [string, ...string[]]).optional(), // Ensures only these values are allowed
  year: z.string(), // Ensures the value is a valid Date object
  startMonth: z.enum([...Months] as [string, ...string[]]).optional(), // Ensures the value is a valid month
  endMonth: z.enum([...Months] as [string, ...string[]]).optional() // Ensures the value is a valid month
    })
})
